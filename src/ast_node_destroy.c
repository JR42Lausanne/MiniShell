...
/*   Updated: 2023/03/30 09:54:10 by graux            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../include/ast.h"

static void	free_agrs(char **args)
{
	int	i;
...
static void	free_content(t_ast_node *node)
{
	if (node->type == AST_CMD)
	{
		...
		free_agrs(((t_cmd_cont *) node->content)->args);
	}
	else
		free_agrs(((t_builtin_cont *) node->content)->args);
	free(node->content);
}
